...
(function() {
	...
	_css("#elrs-config-saver{z-index:1000;position:fixed;top:clamp(8px,2rem,16px);right:clamp(8px,2rem,16px)}.App{text-align:center;border:1px solid #424242;border-radius:8px;gap:8px;padding:8px;display:flex}");
	var DefaultContext = {
		color: void 0,
		size: void 0,
		className: void 0,
	...
	function Tree2Element(tree) {
		return tree && tree.map((node, i) => import_react.createElement(node.tag, _objectSpread({ key: i }, node.attr), Tree2Element(node.child)));
	}
	function GenIcon(data) {
		return (props) => import_react.createElement(IconBase, _extends({ attr: _objectSpread({}, data.attr) }, props), Tree2Element(data.child));
	}
	function IconBase(props) {
		...
		return IconContext !== void 0 ? import_react.createElement(IconContext.Consumer, null, (conf) => elem(conf)) : elem(DefaultContext);
	}
	function MdSave(props) {
		return GenIcon({
			...
			"child": [{
				...
				"tag": "path",
				"attr": { "d": "M17 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14c1.1 0 2-.9 2-2V7zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3m3-10H5V5h10z" },
				"child": []
			}]
		})(props);
	}
	function MdLibraryBooks(props) {
		return GenIcon({
			...
			"child": [{
				...
				"tag": "path",
				"attr": { "d": "M4 6H2v14c0 1.1.9 2 2 2h14v-2H4zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2m-1 9H9V9h10zm-4 4H9v-2h6zm4-8H9V5h10z" },
				"child": []
			}]
		})(props);
	}
	var parseConfiguration = async (file) => {
		const text = await file.text();
		const data = await JSON.parse(text);
		const models = [];
		for (let el of data.models) models.push({
			...el,
			configuration: JSON.parse(el.configuration)
		});
		return models;
	};
	var formatConfig = (configs) => JSON.stringify({ models: configs.map((config) => {
		return {
			id: config.id,
			name: config.name,
			type: config.type,
	...
	var ELRSConfigsContext = (0, import_react.createContext)();
	var ELRSConfigsDispatcherContext = (0, import_react.createContext)();
	var initialConfigs = [];
	function configsReducer(configs, action) {
		switch (action.type) {
			case "init": return action.configs;
			case "add": return [...configs, action.config];
			case "select": return configs.map((c) => {
			...
			case "update": return configs.map((c) => {
				if (c.id === action.config.id) return action.config;
				else return c;
			});
			case "deleted": return configs.filter((c) => c.id !== action.id);
			default: throw Error("Unknown action: " + action.type);
		}
	}
	var ELRSConfigsProvider = ({ children }) => {
		const [configs, dispatch] = (0, import_react.useReducer)(configsReducer, initialConfigs);
		return (0, import_jsx_runtime.jsx)(ELRSConfigsContext.Provider, {
			value: configs,
			children: (0, import_jsx_runtime.jsx)(ELRSConfigsDispatcherContext.Provider, {
				value: dispatch,
				children
			})
		});
	};
	var useELRSConfigsContext = () => (0, import_react.useContext)(ELRSConfigsContext);
	var useELRSConfigsDispatcherContext = () => (0, import_react.useContext)(ELRSConfigsDispatcherContext);
	var ProductTypeContext = (0, import_react.createContext)();
	var ProductTypeProvider = ({ children }) => {
		const [productType, setProductType] = (0, import_react.useState)(null);
		(0, import_react.useEffect)(() => {
			const productNameElement = document.querySelector("#product_name");
			if (productNameElement) {
				const productType = productNameElement.textContent.toLocaleLowerCase().includes("tx") ? "tx" : "rx";
				setProductType(productType);
			}
		}, []);
		return (0, import_jsx_runtime.jsx)(ProductTypeContext.Provider, {
			value: { productType },
			children
		});
	};
	var useProductTypeContext = () => (0, import_react.useContext)(ProductTypeContext);
	_css("button{cursor:pointer;background-color:#1a1a1a;border:1px solid #0000;border-radius:8px;padding:.6em 1.2em;font-family:inherit;font-size:1em;font-weight:500;transition:border-color .25s}button:hover{border-color:#646cff}button:focus,button:focus-visible{outline:none}.danger{color:#fff;background-color:#ff4d4f;border-color:#ff4d4f}.danger:hover{background-color:#ff7875;border-color:#ff4d4f}button:disabled{cursor:not-allowed;opacity:.6;border-color:#0000}");
	var Button = ({ children, onClick, danger, ...props }) => {
	...
	function useModalRoot() {
		...
		(0, import_react.useEffect)(() => {
			...
			setModalRoot(root);
			return () => {
				if (root && root.childNodes.length === 0 && document.body.contains(root)) document.body.removeChild(root);
			};
		}, []);
		return modalRoot;
	}
	function useFocusTrap(ref, isActive) {
		(0, import_react.useEffect)(() => {
			...
			} else if (ref.current.previousActiveElement) ref.current.previousActiveElement.focus();
	...
	function useEscapeHandler(onEscape) {
		(0, import_react.useEffect)(() => {
			if (!onEscape) return;
	...
	var useHandleOutsideClick = (ref, handler) => {
		(0, import_react.useEffect)(() => {
	...
	function useModalAnimation(isOpen, duration = 200, onAnimationEnd = () => {}) {
	...
	_css(".modal-overlay{z-index:1000;background-color:#00000080;justify-content:center;align-items:center;width:100vw;height:100vh;display:flex;position:fixed;top:0;left:0}.modal-content{color:#f7f7f7;background:#424242;border-radius:8px;flex-direction:column;min-width:max(60vw,800px);max-width:500px;min-height:80px;max-height:80vh;animation:.2s ease-out scaleIn;display:flex;position:relative;overflow-y:auto;box-shadow:0 10px 30px #0000004d}.modal-close-button{cursor:pointer;color:#999;background:0 0;border:none;padding:5px;font-size:24px;line-height:1;position:absolute;top:10px;right:10px}.modal-close-button:hover{color:#333}.modal-overlay{animation:.2s ease-out fadeIn}.modal-title{margin:0;padding:20px}.modal-body{flex:1}.modal-footer{padding:20px}@keyframes fadeIn{0%{opacity:0}to{opacity:1}}@keyframes scaleIn{0%{opacity:0;transform:scale(.9)}to{opacity:1;transform:scale(1)}}");
	var Modal = ({ isOpen, onClose, children, title, className = "", footer, footerClassName = "", onClosed, bodyClassName = "" }) => {
		...
		return import_react_dom.createPortal((0, import_jsx_runtime.jsx)("div", {
			...
			children: (0, import_jsx_runtime.jsxs)("div", {
				...
				tabIndex: "-1",
				children: [
					title && (0, import_jsx_runtime.jsx)("h2", {
						id: "modal-title",
	...
	_css(".input{color:#f7f7f7;border:1px solid #ccc;border-radius:4px;outline-style:none;height:36px;padding:8px}");
	var Input = ({ value, onChange, ...props }) => {
	...
	var DownloadConfLib = ({ onDownload }) => {
		...
		const downloadConfigLib = async () => {
			try {
				...
				if (error.name !== "AbortError") console.error("Помилка читання файлу:", error);
			}
		};
		return (0, import_jsx_runtime.jsx)(Button, {
			onClick: async () => {
				if (configs?.length === 0) await downloadConfigLib();
				onDownload?.();
			},
			children: (0, import_jsx_runtime.jsx)(MdLibraryBooks, {})
		});
	};
	var getForm = () => document.querySelector("#upload_options");
	var applyConfigToForm = (selectedConfig) => {
		const form = getForm();
		for (const prop in selectedConfig.configuration) {
			const input = form.querySelector(`[name='${prop}']`);
			if (input) input.value = selectedConfig.configuration[prop];
		}
	};
	var takeFormValues = () => {
		const form = getForm();
	...
	_css(".select-config-modal-body{min-height:300px;margin-top:16px;display:flex}.config-list{min-width:160px}.config-item button{background-color:#424242;border-radius:0;width:100%}.config-item.selected button{background-color:#4d4d4d}.footer{justify-content:space-between;display:flex}.selected-config{background-color:#4d4d4d;flex-grow:1;padding:16px}");
	var SelectConfigModal = ({ isOpen, onClose }) => {
		const configs = useELRSConfigsContext();
		const dispatch = useELRSConfigsDispatcherContext();
		const [selectedConfig, setSelectedConfig] = (0, import_react.useState)(null);
		...
		return (0, import_jsx_runtime.jsxs)(Modal, {
			...
			footer: (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: (0, import_jsx_runtime.jsx)(Button, {
				onClick: handleApply,
				children: "Застосувати"
			}) }),
			children: [(0, import_jsx_runtime.jsx)("ul", {
				className: "config-list",
				children: configs.map((config) => (0, import_jsx_runtime.jsx)("li", {
					...
					children: (0, import_jsx_runtime.jsxs)("button", {
						...
						children: [
							(0, import_jsx_runtime.jsx)("b", { children: config.type }),
							" ",
							(0, import_jsx_runtime.jsx)("span", { children: config.name })
						]
					})
				}, config.id))
			}), (0, import_jsx_runtime.jsx)("div", {
				className: "selected-config",
				children: selectedConfig && (0, import_jsx_runtime.jsx)("pre", { children: JSON.stringify(selectedConfig.configuration, null, 2) })
			})]
		});
	};
	var SelectConfig = () => {
		const [isModalOpen, setIsModalOpen] = (0, import_react.useState)(false);
		return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [(0, import_jsx_runtime.jsx)(DownloadConfLib, { onDownload: () => setIsModalOpen(true) }), (0, import_jsx_runtime.jsx)(SelectConfigModal, {
			isOpen: isModalOpen,
			onClose: () => setIsModalOpen(false)
		})] });
	};
	_css(".save-new-conf{flex-direction:column;gap:8px;padding:0 16px;display:flex}.save-new-conf__footer{justify-content:flex-end;display:flex}.save-new-conf__footer__plural{justify-content:space-between;gap:8px}");
	var SaveConfModal = ({ isOpen, onClose }) => {
		const dispatch = useELRSConfigsDispatcherContext();
		const configs = useELRSConfigsContext() || [];
		const selected = configs.find((config) => config.selected);
		const [updateCurrent, setUpdateCurrent] = (0, import_react.useState)(false);
		...
		const handleSaveNewConf = async (name, updateCurrent) => {
			...
			if (updateCurrent) {
				await write(configs.map((config) => config.id === selected.id ? newConfig : config));
				dispatch({
					type: "update",
					config: newConfig
				});
			} else {
				await write([...configs, newConfig]);
				dispatch({
					type: "add",
					config: newConfig
				});
		...
		(0, import_react.useEffect)(() => {
			if (isOpen && selected) setUpdateCurrent(true);
		}, [isOpen]);
		return (0, import_jsx_runtime.jsxs)(Modal, {
			bodyClassName: "save-new-conf",
			onClose,
			title: selected && updateCurrent ? `Оновити конфігурацію ${selected.name}` : "Введіть назву нової конфігурації",
			isOpen,
			footerClassName: "save-new-conf__footer" + (selected && updateCurrent ? " save-new-conf__footer__plural" : ""),
			footer: (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [selected && updateCurrent && (0, import_jsx_runtime.jsx)(Button, {
				onClick: () => setUpdateCurrent(false),
				children: "Створити нову"
			}), (0, import_jsx_runtime.jsx)(Button, {
				disabled: selected && updateCurrent ? false : name.length < 3,
				onClick: () => handleSaveNewConf(name || selected?.name, updateCurrent),
				children: selected && updateCurrent ? "Оновити" : "Зберегти"
			})] }),
			children: [(!selected || !updateCurrent) && (0, import_jsx_runtime.jsx)(Input, {
				type: "text",
				value: name,
				onChange: (e) => setName(e.target.value),
				placeholder: "Введіть назву нової конфігурації"
			}), configs.length === 0 && (0, import_jsx_runtime.jsxs)("p", { children: [
				"Увага! Ви попередньо не завантажили файл з конфігураціями. При збереженні нової конфігурації в файл ",
				(0, import_jsx_runtime.jsx)("b", { children: "elrs_models.json" }),
				", ви перезапишете існуючі конфігурації які ви попереньо там зберегли."
			] })]
		});
	};
	var SaveConfig = () => {
		...
		return (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [(0, import_jsx_runtime.jsx)(Button, {
			...
			children: (0, import_jsx_runtime.jsx)(MdSave, {})
		}), (0, import_jsx_runtime.jsx)(SaveConfModal, {
			isOpen: isModalOpen,
			onClose: () => setIsModalOpen(false)
		})] });
	};
	function App() {
		return (0, import_jsx_runtime.jsx)(ProductTypeProvider, { children: (0, import_jsx_runtime.jsx)(ELRSConfigsProvider, { children: (0, import_jsx_runtime.jsxs)("div", {
			className: "App",
			children: [(0, import_jsx_runtime.jsx)(SelectConfig, {}), (0, import_jsx_runtime.jsx)(SaveConfig, {})]
		}) }) });
	}
	_css(":root{--lightningcss-light:initial;--lightningcss-dark: ;color-scheme:light dark;color:#ffffffde;font-synthesis:none;text-rendering:optimizelegibility;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-webkit-text-size-adjust:100%;background-color:#242424;font-family:Inter,Avenir,Helvetica,Arial,sans-serif;font-size:16px;font-weight:400;line-height:24px}@media (prefers-color-scheme:dark){:root{--lightningcss-light: ;--lightningcss-dark:initial}}*{box-sizing:border-box;margin:0;padding:0}a{color:#646cff;-webkit-text-decoration:inherit;text-decoration:inherit;font-weight:500}a:hover{color:#535bf2}ul{margin:0;padding:0;list-style:none}");
